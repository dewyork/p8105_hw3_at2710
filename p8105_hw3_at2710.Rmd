...
# health_overall %>% filter(year == 2002) %>% count(state, county) %>% 
 # group_by(state) %>% summarise(number = n()) %>% filter(number == 7) %>% kable()
...
`r health_overall %>% group_by(year, state) %>% count(state) %>% ungroup() %>%  top_n(2)`
